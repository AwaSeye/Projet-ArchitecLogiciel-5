...
<div class="container">
    <?php
    if (!isset($_POST['titre'], $_POST['contenu'], $_POST['dateCreation']))
    {?>
        <br>
        <form action="" method="post">
            <div class="form-group">
                <label for="titre">Titre:</label>
                <input type="texte" name="titre" class="form-control" id="titre" placeholder="Entrer titre" value="<?php if (isset($titre)) { echo $titre ; } ?>">
            </div>
            <div class="form-group">
                <label for="contenu">Contenu:</label><br>
                <textarea name="contenu" id="contenu" cols="80" rows="13" placeholder="Entrer le contenu" value="<?php if (isset($contenu)) { echo $contenu ; } ?>"></textarea>
            </div>
            <div class="form-group">
                <label for="dateCreation">Date Creation:</label>
                <input type="date" name="dateCreation" class="form-control" id="dateCreation" placeholder="Entrer dateCreation" value="<?php if (isset($dateCreation)) { echo $dateCreation ; } ?>">
            </div>
            <?php
    ...
    else
    {
        ...
        {
            $resultat = $client->__soapCall('ajouterArticle', array($parameters));
